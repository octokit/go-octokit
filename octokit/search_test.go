...
func TestSearchService_Users(t *testing.T) {
	...
	assert.EqualValues(t, 2, searchResults.TotalCount)
	assert.EqualValues(t, 2, len(searchResults.Items))
	assert.EqualValues(t, 3338555, searchResults.Items[0].ID)
	assert.EqualValues(t, "dhruvsinghal", searchResults.Items[0].Login)
	assert.EqualValues(t, 9294878, searchResults.Items[1].ID)
...
func TestSearchService_Issues(t *testing.T) {
	...
	assert.False(t, result.HasError())
	assert.False(t, searchResults.IncompleteResults)
	assert.EqualValues(t, 180172, searchResults.TotalCount)
	assert.EqualValues(t, 1551, searchResults.Items[0].Number)
	assert.EqualValues(t, "Colorizer", searchResults.Items[0].Title)
	assert.EqualValues(t, 3402, searchResults.Items[1].Number)
...
func TestSearchService_Repositories(t *testing.T) {
	...
	assert.False(t, result.HasError())
	assert.False(t, searchResults.IncompleteResults)
	assert.EqualValues(t, 21, searchResults.TotalCount)
	assert.EqualValues(t, 21, len(searchResults.Items))
	assert.EqualValues(t, 8269299, searchResults.Items[0].ID)
	assert.EqualValues(t, "ysai/asdfghjk", searchResults.Items[0].FullName)
	assert.EqualValues(t, 8511889, searchResults.Items[1].ID)
...
func TestSearchService_Code(t *testing.T) {
	...
	assert.False(t, result.HasError())
	assert.False(t, searchResults.IncompleteResults)
	assert.EqualValues(t, 7, searchResults.TotalCount)
	assert.EqualValues(t, 7, len(searchResults.Items))
	...
	assert.EqualValues(t, 167174, searchResults.Items[0].Repository.ID)
	assert.EqualValues(t,
